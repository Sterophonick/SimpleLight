...
void Show_ICON_filename(u32 show_offset,u32 file_select,u32 haveThumbnail)
{
    ...
    for(line=0; line<need_show_folder; line++) {
        ...
        if(line== file_select) {
            Clear(17,20 + file_select*14,((char_num == 17)?(17*6+1):(240-17))-2,13,gl_color_selectBG_sd,1);
        }
    ...
    for(line=need_show_folder; line < need_show_folder+need_show_game; line++) {
		...
		else if(!strcasecmp(&(pfilename[strlen8-2]), "mb")) { //GBA (multiboot)
            icon = (u16*)(gImage_icons+1*16*14*2);
        }
...
void IWRAM_CODE Refresh_filename(u32 show_offset,u32 file_select,u32 updown,u32 haveThumbnail)
{
    ...
    if(updown ==2) { //down
        ...
        Clear(17,20 + xx2*14,clean_len2-2,13,gl_color_selectBG_sd,1);
    }
    else { // if(updown ==3)//up
        xx1 = file_select;
        xx2 = file_select+1;
        ...
        Clear(17,20 + xx1*14,clean_len1-2,13,gl_color_selectBG_sd,1);
    ...
    if(shift > timeout) {
        ...
        if(namelen >(char_num-1) ) {
            ...
            if(orgtt!= tt ) {
                ...
                Clear(17,20 + file_select*14,((char_num)*6)-2,13,gl_color_selectBG_sd,1);
...
u32  get_count(void)
{
    ...
    u32 count=0;
    u8 buf[512];
    res = f_open(&gfile,"/SYSTEM/SAVER/Recently play.txt", FA_READ);
...
u32 IWRAM_CODE Loadfile2PSRAM(TCHAR *filename)
{
    ...
    if(res == FR_OK) {
        ...
        DrawHZText12(gl_copying_data,0,78,160-15,gl_color_text,1);
        f_lseek(&gfile, 0x0000);
        ...
        if(page_num==NOR_list) { //boot nor game
            ...
            switch(MENU_line) {
                case 0://DirectPSRAM CLEAN BOOT
                    ...
                    //wait_btn();
                    SetRompageWithHardReset(0x200,1);
                    break;
