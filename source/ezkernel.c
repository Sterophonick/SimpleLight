...
void Show_ICON_filename(u32 show_offset, u32 file_select, u32 haveThumbnail)
{
	...
	for (line = need_show_folder; line < need_show_folder + need_show_game; line++)
	{
		...
		else if (!strcasecmp(&(pfilename[strlen8 - 2]), "mb"))
		{ // PogoShell Plugin/Multiboot image
			icon = (u16 *)(gImage_icon_EXE);
		}
		else if (!strcasecmp(&(pfilename[strlen8 - 3]), "mbz"))
		{ // Compressed PogoShell Plugin
			icon = (u16 *)(gImage_icon_EXE);
		}
		else if (!strcasecmp(&(pfilename[strlen8 - 4]), "mbap"))
		{ // Compressed PogoShell Plugin
			icon = (u16 *)(gImage_icon_EXE);
		}
		else if (!strcasecmp(&(pfilename[strlen8 - 3]), "agb"))
		{ // GBA
			icon = (u16 *)(gImage_icons + 1 * 16 * 14 * 2);
		}
		...
		else if (!strcasecmp(&(pfilename[strlen8 - 2]), "sg"))
		{ // SG-1000
			icon = (u16 *)(gImage_icon_SG);
		}
		else if (!strcasecmp(&(pfilename[strlen8 - 3]), "ngp"))
		{ // Neo Geo Pocket
			icon = (u16 *)(gImage_icon_NG);
		}
		else if (!strcasecmp(&(pfilename[strlen8 - 4]), "ngpc"))
		{ // Neo Geo Pocket Color
			icon = (u16 *)(gImage_icon_NG);
		}
		else if (!strcasecmp(&(pfilename[strlen8 - 3]), "jpg"))
		{ // JPEG Image
			icon = (u16 *)(gImage_icon_IMG);
		}
		else if (!strcasecmp(&(pfilename[strlen8 - 4]), "jpeg"))
		{ // JPEG Image
			icon = (u16 *)(gImage_icon_IMG);
		}
		else if (!strcasecmp(&(pfilename[strlen8 - 3]), "bmp"))
		{ // BMP Image
			icon = (u16 *)(gImage_icon_IMG);
		}
		else if (!strcasecmp(&(pfilename[strlen8 - 3]), "txt"))
		{ // Text Document
			icon = (u16 *)(gImage_icon_TXT);
		}
		else if (!strcasecmp(&(pfilename[strlen8 - 3]), "esv"))
		{ // Fixes the bug with esv files looking like watara supervision
			icon = (u16 *)(gImage_icons + 2 * 16 * 14 * 2);
		}
		else if (!strcasecmp(&(pfilename[strlen8 - 2]), "sv"))
		{ // Watara Supervision
			icon = (u16 *)(gImage_icon_SV);
		}
		else if (!strcasecmp(&(pfilename[strlen8 - 2]), "ws"))
		{ // Wonderswan
			icon = (u16 *)(gImage_icon_WS);
		}
		else if (!strcasecmp(&(pfilename[strlen8 - 3]), "wsc"))
		{ // Wonderswan Color
			icon = (u16 *)(gImage_icon_WS);
		}
		else if (!strcasecmp(&(pfilename[strlen8 - 3]), "col"))
		{ // ColecoVision
			icon = (u16 *)(gImage_icon_CV);
		}
		else if (!strcasecmp(&(pfilename[strlen8 - 3]), "rom"))
		{ // MSX-1
			icon = (u16 *)(gImage_icon_MSX);
		}
		else if (!strcasecmp(&(pfilename[strlen8 - 3]), "pce"))
		{ // PC-Engine
			icon = (u16 *)(gImage_icon_PCE);
		}
		else if (!strcasecmp(&(pfilename[strlen8 - 3]), "z80"))
		{ // Sinclair ZX-Spectrum (Z80)
			icon = (u16 *)(gImage_icon_ZX);
		}
		else if (!strcasecmp(&(pfilename[strlen8 - 2]), "o2"))
		{ // Magnavox Odyssey2 (No emu yet, but I'm eventually going to have a finished one. :D)
			icon = (u16 *)(gImage_icon_o2);
		}
		else if (!strcasecmp(&(pfilename[strlen8 - 2]), "c8"))
		{ // Chip-8
			icon = (u16 *)(gImage_icon_chip);
		}
		else if (!strcasecmp(&(pfilename[strlen8 - 3]), "ch8"))
		{ // Chip-8
			icon = (u16 *)(gImage_icon_chip);
		}
		else if (!strcasecmp(&(pfilename[strlen8 - 3]), "min"))
		{ // Pokemon Mini (No Emu yet)
			icon = (u16 *)(gImage_icon_pokem);
		}
		else if (!strcasecmp(&(pfilename[strlen8 - 3]), "dci"))
		{ // Visual Memory Unit (No Emu yet, but I will make one at some point)
			icon = (u16 *)(gImage_icon_vmu);
		}
		else if (!strcasecmp(&(pfilename[strlen8 - 3]), "vmi"))
		{ // Visual Memory Unit (No Emu yet, but I will make one at some point)
			icon = (u16 *)(gImage_icon_vmu);
		}
		else if (!strcasecmp(&(pfilename[strlen8 - 3]), "mid"))
		{ // MIDI Sequence
			icon = (u16 *)(gImage_icon_mod);
		}
		else if (!strcasecmp(&(pfilename[strlen8 - 3]), "wav"))
		{ // Wave Sound
			icon = (u16 *)(gImage_icon_wav);
		}
		else if (!strcasecmp(&(pfilename[strlen8 - 3]), "nsf"))
		{ // NSF sound file
			icon = (u16 *)(gImage_icon_mod);
		}
		else if (!strcasecmp(&(pfilename[strlen8 - 3]), "k3m"))
		{ // Krawall Advance Module
			icon = (u16 *)(gImage_icon_mod);
		}
		else if (!strcasecmp(&(pfilename[strlen8 - 3]), "mod"))
		{ // Protracker mod file
			icon = (u16 *)(gImage_icon_mod);
		}
		else if (!strcasecmp(&(pfilename[strlen8 - 3]), "pcx"))
		{ // ZSoft Paintbrush PCX image
			icon = (u16 *)(gImage_icon_IMG);
		}
		else if (!strcasecmp(&(pfilename[strlen8 - 3]), "vgm"))
		{ // SMS/GG VGM Rip
			icon = (u16 *)(gImage_icon_mod);
		}
		else if (!strcasecmp(&(pfilename[strlen8 - 3]), "cwz"))
		{ // Unknown Music file, contained in a package for PogoShell 1.2
			icon = (u16 *)(gImage_icon_mod);
		}
		else if (!strcasecmp(&(pfilename[strlen8 - 2]), "sb"))
		{ // MaxMod SoundBank
			icon = (u16 *)(gImage_icon_mod);
		}
		else if (!strcasecmp(&(pfilename[strlen8 - 2]), "ap"))
		{ // aPlib compressed Mode 3 Bitmap
			icon = (u16 *)(gImage_icon_IMG);
		}
		else if (!strcasecmp(&(pfilename[strlen8 - 2]), "lz"))
		{ // LZ77 Compressed Mode 3 Bitmap
			icon = (u16 *)(gImage_icon_IMG);
		}
		else if (!strcasecmp(&(pfilename[strlen8 - 3]), "bgf"))
		{ // BoyScout module
			icon = (u16 *)(gImage_icon_mod);
		}
		else if (!strcasecmp(&(pfilename[strlen8 - 3]), "arc"))
		{ // 4kb Arcadia 2001 ROM File
			icon = (u16 *)(gImage_icon_arc);
		}
		else if (!strcasecmp(&(pfilename[strlen8 - 3]), "a26"))
		{ // Atari 2600 ROM file (emu indev)
			icon = (u16 *)(gImage_icon_a26);
		}
		else if (!strcasecmp(&(pfilename[strlen8 - 2]), "sc"))
		{ // Sega SC-3000 ROM File
			icon = (u16 *)(gImage_icon_SC3000);
		}
		else if (!strcasecmp(&(pfilename[strlen8 - 3]), "mda"))
		{ // Sharp X68000 music
			icon = (u16 *)(gImage_icon_wav);
		}
		else
...
void CheckSwitch(void)
{
	...
	gl_quickboot_sel = Read_SET_info(17);
...
void save_set_info_SELECT(void)
{
	...
	SET_info_buffer[17] = gl_quickboot_sel;
	// save to nor
...
re_showfile:
			...
			else if (keysdown & KEY_SELECT)
			{
				...
				while (1)
				{
					if (re_menu == 1)
					{
						...
						if (gl_quickboot_sel)
							DrawHZText12("(ON)", 32, 47 + (6 * 20), 86, gl_color_text, 1);
						else
							DrawHZText12("(OFF)", 32, 47 + (6 * 20), 86, gl_color_text, 1);
						...
						{
							if (gl_quickboot_sel)
								DrawHZText12("(ON)", 32, 47 + (6 * 20), 86, name_color, 1);
							else
								DrawHZText12("(OFF)", 32, 47 + (6 * 20), 86, name_color, 1);
						}
						re_menu = 0;
					}
					re_menu = 0;
					...
					if (keysdown & KEY_A)
					{
						...
						{
							gl_quickboot_sel = !gl_quickboot_sel;
							save_set_info_SELECT();
							updata = 1;
							Refresh_filename(show_offset, file_select, updata, gl_show_Thumbnail && is_GBA);
							goto refind_file;
	...
	load_file:
		...
		if (is_EMU)
		{
			...
			int bootmode = ((is_EMU > 3) && (is_EMU < 9)) ? ((is_EMU == 6)	 ? 2
															 : (is_EMU == 7) ? 4
																			 : ((is_EMU == 8) ? 5 : 3))
														  : gl_toggle_reset;
